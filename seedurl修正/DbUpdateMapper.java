  ...
  private void addScoreData(WebPage page, Map<CharSequence, CharSequence> map) {
    if (map != null) {
      for (Entry<CharSequence, CharSequence> e : map.entrySet()) {
        ...
        CharSequence seedUrlUtf8 = page.getMarkers().get(DbUpdaterJob.SEED_URL);
        if (depthUtf8 != null)
          depth = Integer.parseInt(depthUtf8.toString());
        ScoreDatum scoreDatum = new ScoreDatum(0.0f, e.getKey().toString(), e.getValue()
                .toString(), depth);
        if (seedUrlUtf8 != null) {
        	scoreDatum.setMeta(DbUpdaterJob.SEED_URL.toString(), seedUrlUtf8.toString().getBytes());
        }
        scoreData.add(scoreDatum);
      }
    }
  }
